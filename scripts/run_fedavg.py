...
NUM_CLIENTS   = int(os.getenv("NUM_CLIENTS", "4"))     # small default
ALPHA         = float(os.getenv("ALPHA", "0.5"))
NUM_ROUNDS    = int(os.getenv("NUM_ROUNDS", "3"))      # small default
LOCAL_EPOCHS  = int(os.getenv("LOCAL_EPOCHS", "1"))
LR            = float(os.getenv("LR", "0.01"))
FRAC_CLIENTS  = float(os.getenv("FRAC_CLIENTS", "0.5"))
BATCH_SIZE    = int(os.getenv("BATCH_SIZE", "32"))     # small default
...
def main():
    ...
    # Only write the header if the CSV does not already exist. This avoids
    # accidentally overwriting previous runs when the same RUN_NAME is used.
    header = [
    ...
    # 3. Build client loaders for full data
    ...
    # 4. Initialize global model (ResNet-18 for CIFAR)
    ...
    for rnd in range(1, NUM_ROUNDS + 1):
        ...
        for cid in selected_clients:
            ...
            # choose loader: full data or coreset
            if USE_CORESET:
            ...
            # 🔧 this part was missing before — actually perform local training
